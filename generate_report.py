...
#api get request
content_web = requests.get(artifactory_uri + "/api/search/prop?build_type=" + build_type + "&repos=" + artifactory_repo, auth=requests.auth.HTTPBasicAuth(user, passwd))
content_web = content_web.json()
# add data to array_web
for i in content_web["results"]:
    array_web.append(str(i["uri"]))

# In this loop, it'schecked that branch name! = Release and the result is written to the array array_web_branch_properties
for i in array_web:
    content_web_branch_properties = requests.get( i + "?properties=branch_name", auth=requests.auth.HTTPBasicAuth(user, passwd))
...
# In this loop, I collect additional parameters of a specific artifact and add them to the object in array_artifact_properties
